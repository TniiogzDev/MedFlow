...
from django.db.models import Q, Subquery, OuterRef, F                         # Para consultas complejas en la base de datos
from django.db.models.functions import Coalesce, Greatest                     # Se añaden funciones de base de datos
...
def obtener_formularios_pendientes(request):
    try:
        ...
        # Base queryset
        formularios_queryset = FormularioAtencion.objects.filter(
            estado__in=['PENDIENTE', 'REQUIERE_MODIFICACION', 'MODIFICADO']
        ).select_related('revisado_por', 'creado_por').order_by('-creado_en') 

        # --- Lógica de Chat para Recepción ---
        User = get_user_model()
        try:
            ambulancia_group = Group.objects.get(name='Ambulancia')
        except Group.DoesNotExist:
            ambulancia_group = None

        # 1. Subquery para el último mensaje de Ambulancia (si el grupo existe)
        if ambulancia_group:
            last_ambulancia_message_subquery = Subquery(
                MensajeChat.objects.filter(
                    formulario_id=OuterRef('id'),
                    autor__groups=ambulancia_group
                ).order_by('-timestamp')
                .values('timestamp')[:1]
            )
            formularios_queryset = formularios_queryset.annotate(
                ultimo_mensaje_ambulancia=last_ambulancia_message_subquery
            )
            
        # Aplicar Paginador
        paginator = Paginator(formularios_queryset, PER_PAGE)
        
        try:
            formularios_pagina = paginator.page(page_number)
        except PageNotAnInteger:
            formularios_pagina = paginator.page(1)
        except EmptyPage:
            formularios_pagina = paginator.page(paginator.num_pages)
            
        lista_formularios = []
        last_viewed_chats = request.session.get('last_viewed_chats', {})
        
        for form in formularios_pagina: # Iterar sobre la página
            has_unread_message = False
            
            # Verificar si hay mensajes sin leer de Ambulancia
            if hasattr(form, 'ultimo_mensaje_ambulancia') and form.ultimo_mensaje_ambulancia:
                form_id_str = str(form.id)
                last_message_time = form.ultimo_mensaje_ambulancia
                last_viewed_timestamp = last_viewed_chats.get(form_id_str)
                
                if last_message_time:
                    if last_viewed_timestamp:
                        try:
                            last_viewed_datetime = timezone.datetime.fromisoformat(last_viewed_timestamp).replace(tzinfo=None)
                            db_time = last_message_time.replace(tzinfo=None) if last_message_time.tzinfo else last_message_time
                            if db_time > last_viewed_datetime:
            ...
            lista_formularios.append({
                ...
                'has_unread_message': has_unread_message, # <--- AÑADIDO PARA CHAT
            })
        
        # Devolver data de paginación
        return JsonResponse({ 
            'status': 'exito', 
            'formularios': lista_formularios,
            'total_paginas': paginator.num_pages,
            'pagina_actual': formularios_pagina.number
        })
    except Exception as e:
...
def obtener_formularios_aprobados(request):
    ...
    # Base queryset
    formularios_queryset = FormularioAtencion.objects.filter(estado='APROBADO').order_by('-creado_en')
...
def obtener_formularios_ambulancia(request):
    ...
    try:
        ...
        # ... (logic for cleaning up locked forms)

        # 1. Subquery para el timestamp del último mensaje (de CUALQUIERA)
        ultimo_mensaje_subquery = Subquery(
            MensajeChat.objects.filter(
                formulario_id=OuterRef('id')
            ).order_by('-timestamp')
            .values('timestamp')[:1]
        ...
        # 2. Obtener la fecha del último mensaje NO enviado por el usuario actual
        formularios = FormularioAtencion.objects.filter(
            creado_por=request.user
        ).select_related('revisado_por').annotate(
            ultimo_mensaje_no_propio=Subquery(
                MensajeChat.objects.filter(
                    formulario_id=OuterRef('id')
                ).exclude(autor=request.user) # Excluye mensajes propios
                .order_by('-timestamp')
                .values('timestamp')[:1]
            )
        # --- LÓGICA DE ORDENAMIENTO POR ACTIVIDAD ---
        ).annotate(
            # Calcula el timestamp más reciente entre la creación, aprobación y el último chat
            ultima_actividad=Greatest(
                F('creado_en'),
                Coalesce(F('aprobado_en'), F('creado_en')),
                Coalesce(ultimo_mensaje_subquery, F('creado_en')) 
            )
        ).order_by('-ultima_actividad') # Ordena por la actividad más reciente

        # 3. Leer el timestamp de la última vista del chat (almacenado en la sesión)
        last_viewed_chats = request.session.get('last_viewed_chats', {})
        ...
        for form in formularios:
            ...
            # Determinar si hay mensajes sin leer
            has_unread_message = False
            last_message_time = form.ultimo_mensaje_no_propio
            
            if last_message_time:
                # Comprobar si el último mensaje NO propio fue enviado DESPUÉS de la última vista
                last_viewed_timestamp = last_viewed_chats.get(form_id_str)
                
                if last_viewed_timestamp:
                    try:
                        # Convertir timestamp de la sesión a datetime (debe coincidir con la zona horaria)
                        last_viewed_datetime = timezone.datetime.fromisoformat(last_viewed_timestamp).replace(tzinfo=None)
                        
                        # Aseguramos que el timestamp de la DB tenga o no TZ para la comparación
                        db_time = last_message_time.replace(tzinfo=None) if last_message_time.tzinfo else last_message_time
                        
                        if db_time > last_viewed_datetime:
                            has_unread_message = True
                    except ValueError:
                        # Si el formato en la sesión es inválido, forzamos la lectura
                        has_unread_message = True
                else:
                    # Si nunca ha sido visto, se considera no leído
                    has_unread_message = True
            ...
            lista_formularios.append({
                ...
                'has_unread_message': has_unread_message, # <-- NUEVA BANDERA
            })
...
# API: Marcar Chat como leído (NUEVA FUNCIÓN CRUCIAL)
@login_required
def marcar_chat_leido(request, form_id):
    if request.method == 'POST':
        try:
            # 1. Obtener la hora actual en formato ISO
            now_iso = timezone.now().isoformat()
            
            # 2. Obtener el diccionario de chats vistos de la sesión
            # Nota: Esto crea el diccionario si no existe.
            last_viewed_chats = request.session.get('last_viewed_chats', {})
            
            # 3. Marcar este formulario con el tiempo actual
            last_viewed_chats[form_id] = now_iso
            
            # 4. Guardar en la sesión (requiere que la sesión sea modificada explícitamente)
            request.session['last_viewed_chats'] = last_viewed_chats
            request.session.modified = True
            
            return JsonResponse({'status': 'exito', 'message': 'Chat marcado como leído.'})
        except Exception as e:
            return JsonResponse({'status': 'error', 'message': str(e)}, status=500)
    return JsonResponse({'status': 'error', 'message': 'Método no permitido'}, status=405)
...
def generar_excel_actualizado(response, formulario):
    ...
    if formulario.aprobado_por:
        ...
        # CORRECCIÓN DE SEGURIDAD: Verifica antes de formatear
        fecha_aprobacion_str = timezone.localtime(formulario.aprobado_en).strftime('%d-%m-%Y %H:%M') if formulario.aprobado_en else "N/A"
        add_row("Fecha Aprobación:", fecha_aprobacion_str, row); row += 1
...
def generar_pdf(response, formulario):
    ...
    # Asegura la conversión y el formateo de fecha de creación
    fecha_creacion_display = timezone.localtime(formulario.creado_en).strftime('%d-%m-%Y %H:%M') if formulario.creado_en else "N/A"
    c.write_field("Fecha Creación:", fecha_creacion_display)
    
    if formulario.aprobado_por:
        c.write_field("Aprobado Por:", formulario.aprobado_por.get_full_name())
        # CORRECCIÓN DE SEGURIDAD: Verifica antes de formatear
        fecha_aprobacion_str = timezone.localtime(formulario.aprobado_en).strftime('%d-%m-%Y %H:%M') if formulario.aprobado_en else "N/A"
        c.write_field("Fecha Aprobación:", fecha_aprobacion_str)
